...
- 弹窗高度最大化，方便查看所有模型
- 模型名称后显示可用率百分比、平均延迟、路由滑动窗口质量分（蓝色）

#### 📋 调用日志
- 新增"调用日志"标签页，记录每次模型调用的详细数据
- 显示：时间、提供商、模型、路由分、输入/输出/合计 Token、耗时（秒）、输出速度（TPS）
- 失败调用以 ❌ 标记并显示红色
- 支持近 24 小时 / 近 7 天 / 近 30 天切换
- 数据自动保留 30 天，超期自动清理

#### 🔄 路由质量分实时同步
- `record_fail()` / `record_success()` 同步更新 `model_quality` 滑动窗口和 `history.jsonl`
- 真实请求的成败实时影响路由质量分和 UI 统计分
- 启动时从 `history.jsonl` 全量回放到 `model_quality`，重启后路由分与历史一致

#### 🔍 路由分 UI 展示
- 新增 `/api/quality-scores` 接口返回所有模型路由滑动窗口质量分
- 路由面板模型列表显示蓝色路由分，与 UI 统计分并列对比
- 调用日志新增"路由分"列

#### ⏱ 轮询策略优化
- 改为**按模型独立计数**，新加模型不受老模型累积轮询次数影响
- 新增模型首次自动轮询：加入后立即执行首次探测，不打断全局轮询节奏
- 每个模型独立统计轮询次数，满 20 次后单独降频，不影响其他模型
- 所有模型均满 20 次后整体切换到每天 12:00 一次

#### 📊 质量分计算修正
- 滑动窗口分母固定最多 20 条（`deque(maxlen=20)`），不会出现 20/21 的情况

#### 🎲 模型轮换算法（v1.6.1）
- 模型连续使用 **10 次以上 + 闲置 5 分钟** 后，触发一次性随机排序
- 所有候选模型获得临时随机偏移（**-5%~+5%**），仅作用于当次排序，不写入滑动窗口
- 不累积、不干扰质量分和熔断器
- 质量分接近的模型之间自然轮换，避免单个高分模型永久霸占
- 触发后会话计数清空，不会每次请求都重新生成随机值

#### ✅ 就绪状态提示
- 启动时显示"⭕️ 就绪"绿色徽标
- 非工作时间显示"⏰ 非工作时间"提示，不再卡在"初始化中"
- 控制台压制 httpx 请求噪音，保留有用日志

#### ⏱ 延迟惩罚纳入路由分
- 路由综合分 = 质量分 - 延迟惩罚 + 随机偏移
- 延迟惩罚：超过 1 秒的部分每 1 秒扣 2%，**最多扣 0.3 分**，精确到 **4 位小数**
- 修复 `get_avg_latency()` 返回毫秒被当作秒计算的 bug（除以 1000 转为秒）
- 质量分相同时仍以延迟作为第二排序条件

#### 🐛 识图配置勾选修复
- 修复同名模型（同 provider 多 key）在识图配置中勾选/取消无效的问题
- 改用计数逻辑替代 `Set`，与路由面板一致
